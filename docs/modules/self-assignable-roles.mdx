---
sidebar_position: 10
keywords: [Self-Assignable Roles, dashboard, server, module, Embed Message, Add Message, Edit, Embed, Choose a message, Role limitation per user, Button, color, Add Button, Settings, Emoji, text, Colors, Add Roles, Select Menu, Placeholder, Emoji, Label, Option Title, Roles, Reaction, Add Emoji, normal, gif, Role Mode, Toggle Mode, Give Mode, Take Mode, Type of Notification, Notify Type, Silent Type, Important Notes, Premium Tier 1/2, exclusive roles, workflow, message style, configurations, send a message, button interaction, select the menu, member interact, traditional reaction roles, apply roles, several mode options, manage notification type, reply message, mention role, quiet, select menu options, server roles hierarchy, role for the reaction, multiple roles, same option, 25 buttons, single embed message, 20 select menu options, background, linked messages, problems] 
---
...
The text filed field is required; you must fill it out.
