...
With Probots levelling system, active server members will be automatically rewarded with extraordinary level roles, privileged permissions, and access to mystery channels once they reach a specific level!
...
You can prevent certain users with the selected roles from accumulating experience points anywhere on your server by using this.
...
:::important Three potential situations that may arise when setting up a level requimrent
...
- Users need to type in chats for the bot to be able to update their XP data and award them voice-level prizes. Only then will the bot be able to reward them.
