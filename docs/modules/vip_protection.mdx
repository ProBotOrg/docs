...
description: VIP Protection, which provides a secure environment for your server by restricting the actions of users with permissions and limiting their ability to perform certain commands.
keywords: [VIP Protection, ProBot, Server, Premium subscriptions, Actions, Users, Permissions, Role, Discord, Channels, Commands, Mute, Kick, Ban, Bot, Control panel, Administrator, Protection module, Restrictions, Whitelist, Ownership, Community, Limits]
---
...
Note that VIP Protection is a premium feature and is only available to subscribers with a premium subscription. If you do not have a premium subscription, you will need to upgrade to access this feature.
...
Note that when the first two options are selected, the server ownership will receive a backup code in DMs to revert the role permissions.
...
Note that users with administrative permissions will bypass the role options
...
Keep in mind that the limit number is infinity, you can set the desired number with no restriction. 
