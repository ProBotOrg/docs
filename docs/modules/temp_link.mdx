---
sidebar_position: 13
keyword: [Temporary Link Command, server administrators, command, users, invite, friends, dashboard, select, module, enabled, premium subscribers, personalized link, text, prefix, symbol, character, message, reply, successfully sent, DMs, adjusted, bot, separate message, cannot send, closed, roles, prevent, specific, enabled, chosen, channels, respond, information, invite link, expiration time, uses, limit, number, invitation default channel, redirected, age, minutes, expire, linkage, waiting time, hours, requested, enforce, message, bypass, Invite Members, permission, delete, user command invocation, bot's reply, persistent, limits, temp link options] 
---
